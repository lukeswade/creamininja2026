...
router.post(
  ...
  async (c) => {
    ...
    if (exists)
      return c.json(badRequest("Email or handle already in use"), 400);
    ...
    const csrfCookie = serializeCookie("cn_csrf", session.csrfToken, {
      ...cookieOpts(c.env),
      httpOnly: false
    });
    ...
    return c.json(
      jsonOk({
        ok: true,
        user: await getUser(c.env, userId),
...
router.post(
  ...
  async (c) => {
    ...
    return c.json(
      jsonOk({
        ok: true,
        user: await getUser(c.env, user.id),
        csrfToken: session.csrfToken
      }),
      200,
      { "Set-Cookie": [setCookie, csrfCookie] as any }
...
router.get("/google/callback", async (c) => {
  try {
    ...
    if (!profileRes.ok)
      return c.json(badRequest("Google profile fetch failed."), 400);
    ...
    if (!userId) {
      const existingUser = await first<{ id: string }>(
        c.env,
...
router.post("/logout", authOptional, requireAuth, requireCsrf, async (c) => {
  ...
  const token = cookies
    .split(";")
    .map((x) => x.trim())
    .find((x) => x.startsWith("cn_session="))
    ?.split("=")[1];
  if (token) {
  ...
  const clearSession = serializeCookie("cn_session", "", {
    ...cookieOpts(c.env),
    maxAge: 0
  });
  const clearCsrf = serializeCookie("cn_csrf", "", {
    ...cookieOpts(c.env),
    httpOnly: false,
    maxAge: 0
  });

  return c.json(jsonOk({ ok: true }), 200, {
    "Set-Cookie": [clearSession, clearCsrf] as any
  });
});
...
async function createSession(env: Env, userId: string) {
  ...
  await run(
    ...
    [sessionId, userId, tokenHash, csrfToken, expiresAt]
  );
