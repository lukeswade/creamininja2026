...
async function canView(env: Env, viewerId: string | null, recipeId: string): Promise<boolean> {
  const recipe = await first<{ author_id: string; visibility: string }>(env, "SELECT author_id, visibility FROM recipes WHERE id = ?", [recipeId]);
  ...
  if (recipe.visibility === "restricted") {
    const f = await first<{ user_id: string }>(env, "SELECT user_id FROM friendships WHERE user_id = ? AND friend_id = ?", [viewerId, recipe.author_id]);
    return !!f;
  }
  // private
...
router.get("/:id", authOptional, async (c) => {
  ...
  const row = await first<any>(c.env, `${recipeSelect(viewer)} WHERE r.id = ?`, [id]);
...
router.get("/by-handle/:handle", authOptional, async (c) => {
  ...
  const rows = await all<any>(
    ...
    `${recipeSelect(viewer)}
     ...
     LIMIT 50`,
    [handle, viewer, viewer, viewer, viewer, viewer, viewer, viewer]
  );
...
router.post("/", zValidator("json", RecipeCreateSchema), async (c) => {
  ...
  const row = await first<any>(c.env, `${recipeSelect(me.id)} WHERE r.id = ?`, [id]);
  return c.json(jsonOk({ ok: true, recipe: normalize(row) }));
});
...
router.patch("/:id", zValidator("json", RecipeUpdateSchema), async (c) => {
  ...
  if (body.ingredients !== undefined) { fields.push("ingredients_json = ?"); params.push(JSON.stringify(body.ingredients)); }
  if (body.steps !== undefined) { fields.push("steps_json = ?"); params.push(JSON.stringify(body.steps)); }
  if (body.imageKey !== undefined) { fields.push("image_key = ?"); params.push(body.imageKey); }
...
router.post("/:id/share", zValidator("json", ShareSchema), async (c) => {
  ...
  const recipe = await first<{ author_id: string; visibility: string }>(c.env, "SELECT author_id, visibility FROM recipes WHERE id = ?", [id]);
  if (!recipe) return c.json(notFound(), 404);
  if (recipe.author_id !== me.id) return c.json(forbidden("Only author can share"), 403);

  // allow sharing any visibility; private needs share to be accessible
...
router.post("/:id/star", zValidator("json", StarSchema), async (c) => {
  ...
  const res = await c.env.DB.prepare("INSERT OR IGNORE INTO stars (user_id, recipe_id) VALUES (?, ?)").bind(me.id, id).run();
...
router.delete("/:id/star", async (c) => {
  ...
  const del = await c.env.DB.prepare("DELETE FROM stars WHERE user_id = ? AND recipe_id = ?").bind(me.id, id).run();
  if (del.meta.changes && del.meta.changes > 0) {
    await run(c.env, "UPDATE recipes SET stars_count = CASE WHEN stars_count > 0 THEN stars_count - 1 ELSE 0 END WHERE id = ?", [id]);
  }
  return c.json(jsonOk({ ok: true }));
});
